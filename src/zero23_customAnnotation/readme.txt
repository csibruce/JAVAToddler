...
1. 사용자 정의 어노테이션 작성 정보
   ...
   @Target - annotation을 적용 대상 지정.
                  ...
                  ElementType.FIELD : enum상수를 포함한 필드
  ...
  @Retention - annotation 정보의 유효화 설정
                  RententionPolicy.SOURCE : 원본 파일 소스단에서 취득 활용가능한 어노테이션 정의
				  RententionPolicy.CLASS : 컴파일된 바이트코드단에서 취득 활용가능한 어노테이션 정의
                  RententionPolicy.RUNTIME : 실행시 java reflection을 활용해 취득 활용가능한 
                                                                어노테이션 정의
...
2. example
   ...
   ** 어노테이션 인터페이스 작성시 포함되는 함수들은 해당 어노테이션의 필수 속성으로 동작하며, 필수 속성 해제시
   ...
   2.4 @StringValueAnnotation을 클래스 내 전역변수에만 설정 가능하도록 정의하고
          해당 오노테이션 설정시 문자열값을 설정하도록 하고, 해당 어노테이션의 취득은
          실행시 취득 활용되도록 정의 
